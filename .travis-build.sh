...
$builddep -q -y python-PyKrige.spec

if [[ $image =~ ^fedora: || $image =~ ^centos: ]]
then
    mkdir -p ~/rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}
    cp python-PyKrige.spec ~/rpmbuild/SPECS/
    spectool -g -R -S ~/rpmbuild/SPECS/python-PyKrige.spec
    set +x
    rpmbuild -ba ~/rpmbuild/SPECS/python-PyKrige.spec 2>&1 | pv -q -L 3k
else
